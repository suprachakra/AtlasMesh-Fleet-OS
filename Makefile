...
test-sim: ## Run simulation tests
	...
	@cd sim && python -m pytest tests/ -v
...
docker-build: ## Build Docker images
	...
	@docker build -t $(DOCKER_REGISTRY)/trip-service:$(DOCKER_TAG) -f services/trip-service/Dockerfile .
...
docker-push: ## Push Docker images to registry
	...
	@docker push $(DOCKER_REGISTRY)/trip-service:$(DOCKER_TAG)
...
docker-scan: ## Scan Docker images for vulnerabilities
	...
	@docker scout cves $(DOCKER_REGISTRY)/trip-service:$(DOCKER_TAG)

# Deployment Commands
deploy-dev: ## Deploy to development environment
	@echo "$(BLUE)Deploying to development environment...$(RESET)"
	@kubectl apply -f deploy/k8s/dev/ --namespace=atlasmesh-dev

deploy-staging: ## Deploy to staging environment
	@echo "$(BLUE)Deploying to staging environment...$(RESET)"
	@kubectl apply -f deploy/k8s/staging/ --namespace=atlasmesh-staging
...
deploy-prod: ## Deploy to production environment (requires approval)
	...
	@echo "Run: kubectl apply -f deploy/k8s/prod/ --namespace=atlasmesh-prod"
