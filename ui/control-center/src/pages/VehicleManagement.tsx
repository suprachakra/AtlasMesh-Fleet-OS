        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="text-sm text-gray-500">
              {row.original.plate} • {row.original.vinMasked ? 
                `${row.original.vin?.slice(0, 3)}XXXXXX${row.original.vin?.slice(-4)}` : 
                row.original.vin}
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">VMS Dashboard</h1>
            <Select className="ml-4">
              <option value="fleet_operation">Fleet Operation</option>
              <option value="maintenance">Maintenance Mode</option>
              <option value="testing">Testing Mode</option>
              <option value="emergency">Emergency Mode</option>
            </Select>
            <div className="text-sm text-gray-500 ml-4">
              Last login: {new Date().toLocaleString('en-US', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </div>
          </div>
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-medium text-gray-900">
                {filteredVehicles.filter(v => v.groups.includes('main_fleet')).length} Groups | {filteredVehicles.length} Vehicles
              </h2>
