...
interface Trip {
  ...
  classification: 'P0_demo' | 'P1_operation_run' | 'P1_driverless_operation' | 'P1_chery_run' | 'P2_release_run' | 'P3_engineering_run'
  onboardStage: 'with_co_drive' | 'driverless_operation' | 'remote_monitored' | 'safety_chase'
  vehicleId: string
  vehicleName: string
  garagePc?: string
  startTime: Date
  ...
  remoteOperatorEmail?: string
  followingCarDriverEmail?: string
  ...
  onboardFlags: string[]
  onboardSuiteUrl?: string
  prepTime: number
  tripObsoleteDuration: number // minutes
  vpnRequired: boolean
...
const FleetScheduling: React.FC = () => {
  ...
  const [tripForm, setTripForm] = useState({
    ...
    classification: 'P1_operation_run' as 'P0_demo' | 'P1_operation_run' | 'P1_driverless_operation' | 'P1_chery_run' | 'P2_release_run' | 'P3_engineering_run',
    onboardStage: 'with_co_drive' as 'with_co_drive' | 'driverless_operation' | 'remote_monitored' | 'safety_chase',
    vehicleId: '',
    garagePc: '',
    startTime: '',
    duration: 120,
    ...
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        <div className="flex items-center space-x-4 mt-4">
          {/* Related to Me Toggle */}
          <label className="flex items-center space-x-2 px-3 py-2 bg-gray-100 rounded-md">
            <input
              type="checkbox"
              checked={filters.relatedToMe}
              onChange={(e) => setFilters({ ...filters, relatedToMe: e.target.checked })}
              className="rounded border-gray-300"
            />
            <span className="text-sm font-medium text-gray-700">Related to me</span>
          </label>
          ...
          <Select
            value={filters.garagePc}
            onValueChange={(value) => setFilters({ ...filters, garagePc: value })}
          >
            <option value="">All Garages</option>
            <option value="garage_a">Garage A</option>
            <option value="garage_b">Garage B</option>
            <option value="garage_c">Garage C</option>
          </Select>
          ...
          {currentView === 'daily' ? (
            ...
            <div className="p-6">
              ...
              <div className="bg-white rounded-lg border border-gray-200">
                ...
                {/* Vehicle Rows */}
                {mockVehicles.map(vehicle => (
                  <div key={vehicle.id} className="grid grid-cols-8 border-b border-gray-100 last:border-b-0">
                    <div className="p-4 bg-gray-50 border-r border-gray-200">
                      <div className="text-sm font-medium text-gray-900">{vehicle.name}</div>
                    ...
                    {[0, 1, 2, 3, 4, 5, 6].map(dayOffset => {
                      ...
                      dayDate.setDate(selectedDate.getDate() - selectedDate.getDay() + dayOffset + 1)
                      
                      const dayTrips = filteredTrips.filter(trip => 
                        trip.vehicleId === vehicle.id &&
                        trip.startTime.toDateString() === dayDate.toDateString()
                      )
                            ...
                            {dayTrips.map(trip => (
                              <div
                                ...
                                className={`p-2 rounded text-xs cursor-pointer
                                  ...
                                  ${trip.conflicts.length > 0 ? 'border-red-400' : ''}
                                `}
                              >
                                <div className="font-medium truncate">{trip.businessType}</div>
                                <div className="text-xs opacity-75">
                                  {trip.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </div>
                              </div>
