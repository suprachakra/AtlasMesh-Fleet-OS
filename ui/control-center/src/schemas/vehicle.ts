...
// Vehicle state schema (for VMS dashboard filtering)
export const vehicleStateSchema = z.enum([
  'unknown',      // Unknown state
  'normal',       // Normal operation
  'defective',    // Defective but operational
  'down',         // Down/non-operational
...
export const vehicleSchema = z.object({
  ...
  vin: z.string().optional(), // Vehicle Identification Number (may be masked)
  vinMasked: z.boolean().default(true), // Whether to mask VIN in UI
  year: z.number().int().min(2000).max(new Date().getFullYear() + 2).optional(),
  
  // Operational status
  operationalStatus: vehicleStatusSchema,
  vehicleState: vehicleStateSchema.default('unknown'), // VMS dashboard state
  ...
  cloudRegion: z.string().optional(), // e.g., "AZURE_SOUTHEASTASIA_SINGAPORE"
  chargeableCities: z.array(z.string()).default([]), // Cities where vehicle can operate (billing)
  operateableCities: z.array(z.string()).default([]), // Cities with operational permits
  ...
  simCard: z.object({
    ...
    dataUsage: z.number().nonnegative().default(0), // GB
    lastHeartbeat: z.string().datetime().optional(),
  }).optional(),
  
  vpnList: z.array(z.object({
    vpnId: z.string(),
    name: z.string(),
  ...
  dataflow: z.object({
    uploading: z.number().nonnegative().default(0), // MB/s
    downloading: z.number().nonnegative().default(0), // MB/s
    totalUploaded: z.number().nonnegative().default(0), // GB
    totalDownloaded: z.number().nonnegative().default(0), // GB
  }).optional(),
