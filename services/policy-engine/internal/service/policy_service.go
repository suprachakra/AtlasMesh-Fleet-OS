...
func (s *PolicyService) evaluatePolicyContent(
	...
	// Implement OPA policy evaluation with proper error handling
	decision, err := s.evaluateWithOPA(evalCtx, policy, input)
	if err != nil {
...
// evaluateWithOPA performs policy evaluation using Open Policy Agent
func (s *PolicyService) evaluateWithOPA(ctx context.Context, policy *models.Policy, input map[string]interface{}) (*models.PolicyDecision, error) {
	// Create OPA query with proper input structure
