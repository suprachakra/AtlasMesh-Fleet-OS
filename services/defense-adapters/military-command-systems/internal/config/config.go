...
// Config holds all configuration for the military command systems adapter
type Config struct {
	Environment string         `json:"environment"`
	Server      ServerConfig   `json:"server"`
	Database    DatabaseConfig `json:"database"`
	Military    MilitaryConfig `json:"military"`
...
// MilitaryConfig holds military-specific configuration
type MilitaryConfig struct {
	CommandSystemURL    string `json:"command_system_url"`
	ClassificationLevel string `json:"classification_level"`
	EncryptionEnabled   bool   `json:"encryption_enabled"`
	AuthenticationType  string `json:"authentication_type"`
	MissionTimeout      int    `json:"mission_timeout_minutes"`
}
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		Database: DatabaseConfig{
			URL:             getEnvString("DATABASE_URL", "postgres://postgres:password@localhost:5432/atlasmesh_defense?sslmode=disable"),
		...
		Military: MilitaryConfig{
			CommandSystemURL:    getEnvString("MILITARY_COMMAND_URL", "https://command.mil.local"),
			ClassificationLevel: getEnvString("CLASSIFICATION_LEVEL", "CONFIDENTIAL"),
			EncryptionEnabled:   getEnvBool("ENCRYPTION_ENABLED", true),
			AuthenticationType:  getEnvString("AUTH_TYPE", "certificate"),
			MissionTimeout:      getEnvInt("MISSION_TIMEOUT_MINUTES", 60),
		},
...
func (c *Config) validate() error {
	...
	if c.Military.CommandSystemURL == "" {
		return fmt.Errorf("military command system URL is required")
	}
	...
	for _, valid := range validClassifications {
		if c.Military.ClassificationLevel == valid {
			isValidClassification = true
			break
		}
	}
	if !isValidClassification {
		return fmt.Errorf("invalid classification level: %s", c.Military.ClassificationLevel)
	}
